...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock,
                                    const Consensus::Params& params, int nPoWType)
{
    ...
    if (!nPoWType && (pindexLast->GetBlockTime() >= Params().PowUpdateTimestamp())) {
    ...
    return DarkGravityWave(pindexLast, params, nPoWType);
}
...
unsigned int DarkGravityWave(const CBlockIndex* pindexLast, const Consensus::Params& params, int nPoWType) {
    ...
    int64_t nPastBlocks = Params().GetDwgPastBlocks(pindexLast, nPoWType);
    bool fNewPoW = true;
    while (nCountBlocks < nPastBlocks) {
        ...
        if ((nPoWType & CBlockHeader::RANDOMX_BLOCK) && !pindex->IsRandomXProofOfWork()) {
            pindex = pindex->pprev;
            continue;
    ...
    if (pindexLastMatchingProof && !pindex->IsRandomXProofOfWork()) {
    ...
    int64_t nPowSpacing  = Params().GetTargetSpacing(pindexLast, nPoWType);
    ...
    LogPrint(BCLog::BLOCKCREATION, "%s: nActualTimespan=%d, nTargetTimespan=%d Overdue=%d (%s)\n", __func__,
             nActualTimespan, nTargetTimespan, nOverdueTime, GetMiningType(nPoWType).c_str());
    // ***
    ...
    if ((nActualTimespan < nTargetTimespan) && (nOverdueTime > 0)) {
        ...
        LogPrint(BCLog::BLOCKCREATION, "%s: %s is ahead but now overdue, don't adjust anymore.\n",
                 __func__, GetMiningType(nPoWType).c_str());
    ...
    LogPrint(BCLog::BLOCKCREATION, "%s: Adjusting %s: old target: %s\n",
             __func__, GetMiningType(nPoWType).c_str(), bnNew.GetHex());
...
uint256 RandomXHashToUint256(const char* p_char) {
    ...
    std::string hexStr = HexStr(Span<const unsigned char>(vec.data(), vec.size()));
    return uint256S(hexStr);
}
